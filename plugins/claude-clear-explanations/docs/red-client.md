# Claude adapter red evidence

Command: `node --test plugins/claude-clear-explanations/tests/claude-client.test.mjs`

Exit status: 1. Six tests failed; no tests passed.

The contract scaffold returned `undefined`. No production behavior existed before this run. The first assertion expected structured output and accounting, but received `undefined`. Error-path tests reported `Missing expected rejection` for invalid output, transport failure, cancellation, and invalid input. The options test could not obtain captured options because the scaffold never called the injected query. Imports and fixtures loaded. The failures show absent adapter behavior.

This is the frozen test snapshot. The whole file is new; every line is an addition.
...
The same command passed all 6 frozen tests with exit status 0. `node --check plugins/claude-clear-explanations/src/claude-client.mjs` also passed. No live API call was made.

## SDK reference and verification limits

Use `@anthropic-ai/claude-agent-sdk` version `0.3.273`, verified in the official [release](https://github.com/anthropics/claude-agent-sdk-typescript/releases/tag/v0.3.273). The registry lookup did not complete and was stopped.

The [TypeScript reference](https://code.claude.com/docs/en/agent-sdk/typescript) documents query options and result envelopes. The [settings guide](https://code.claude.com/docs/en/agent-sdk/claude-code-features) explains why empty setting sources also need a separate config directory and disabled automatic memory. Managed host policy still applies. The [structured-output guide](https://code.claude.com/docs/en/agent-sdk/structured-outputs) documents schema validation and failure results. Tests verify the adapter options and stream handling through an injected query; they do not prove live model behavior or SDK internal tool behavior. The controller must validate the returned object against its application schema before use.
